...
bl_info= {
    "name": "Imports/Exports MT Framework Mobile Models",
    "author": "Converted from MH4U Importer from Seth VanHeulen",
    "version": (1, 3),
    "blender": (3, 6, 0),
    "location": "File > Import > MTF Mobile Model (.mod)",
    "description": "Imports/Exports MT Framework Mobile Models.",
...
# 11/9: 1.2, Aman, remove useless color layer
# 11/10 1.3, Aman, accept both mod version 0xe5 and 0xe6
import array
import struct
...
def build_vertex(vertex, UV, NTB , bonemap):
    groupWeights = {}
    x = vertex.co[0]
    z = -vertex.co[1]
    y = vertex.co[2]
...
def load_mod(filename, context):
    ...
    if mod_header[0] != b'MOD\x00' or (  mod_header[1] != 0xe5 and mod_header[1] !=0xe6) :
        print ("wrong header")
        mod.close()
        return    
    ...
    for m in range(meshCnt):
        ...
        if m in active_mesh:  # this mesh for keeping
            ...
            for i,v in enumerate(editedMesh.vertices):
                raw_vertex = build_vertex(v,UVs[i],NTB[i],BoneMapArray[0])
                nf.write(raw_vertex)
...
class IMPORT_OT_mod(bpy.types.Operator):
    ...
    bl_label = "Import MOD"
    bl_description = "Import a MT Framework Mobile model"
...
class EXPORT_OT_mod(bpy.types.Operator):
    ...
    bl_label = "Export MT Framework Mobile model"
...
def menu_func(self, context):
    self.layout.operator(IMPORT_OT_mod.bl_idname, text="MT Framework Mobile Model (.mod)")

def export_menu_func(self, context):
    self.layout.operator(EXPORT_OT_mod.bl_idname, text="MT Framework Mobile Model (.mod)")
